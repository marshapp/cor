program euler_explicit
    ...
    real :: delta_t, alpha, lambda, P_ext, beta, gamma(3), R(0:N_z-1,3), delta_z
    real, parameter :: L = 0.02, V = 40, c_v = 3686, rho = 1081, k = 0.56, sigma = 0.472, T_c=36.5, t_a=0.025
    real, allocatable :: T(:,:) 

    delta_z = 1/(real(N_z)-1)
    ...
    do m=1, 3
        ...
        delta_t = gamma(m)*(delta_z**2)

        N_t=int(t_a/delta_t)

        allocate(T(0:N_z-1, 0:N_t))
    ...
    do j = 0, N_z-1
        write(u, *) (j*delta_z)*L, R(j,1), R(j,2), R(j,3), f(200, j*delta_z)
    end do
    ...
    do j = 0, N_z-1
        write(w, *) (j*delta_z)*L, abs(R(j,2)-f(200, j*delta_z)), abs(R(j,3)-f(200, j*delta_z))
    end do
    ...
    contains
        ...
        real function f(N, z_norm)
            ...
            do k = 1, N
                sumatori = sumatori + ((1-exp(-((2*k-1)**2)*(pi**2)*t_norm))/((2*k-1)**3))*sin((2*k-1)*pi*z_norm)
            end do
